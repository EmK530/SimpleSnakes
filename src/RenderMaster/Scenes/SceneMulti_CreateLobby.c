...
{
    SceneMulti_CreateLobby* scene = malloc(sizeof(SceneMulti_CreateLobby));
    scene->sceneInfo = malloc(sizeof(GenericScene));
    scene->sceneInfo->context = scene;
    scene->sceneInfo->prepFunction = SceneMulti_CreateLobby_Prepare;
    scene->sceneInfo->workFunction = SceneMulti_CreateLobby_Work;
    scene->sceneInfo->inputFunction = SceneMulti_CreateLobby_OnInput;
    ...
    returnData->sceneId = SCENE_MULTI_CREATELOBBY;
    returnData->sceneName = strdup("Multiplayer_CreateLobby");
...
{
    SceneMulti_CreateLobby* self = (SceneMulti_CreateLobby*)_self;
            ...
            case Enter:
            {
                switch(self->selPos)
                {
                    case 2: // Create Public
                    {
                        ...
                        int result = multiplayer_host(mp, self->nameInput, 0);
                        if(result != MPAPI_OK)
                        {
                            AudioManager_PlayCh("assets/audio/sfx/denied.ogg", 0.5, 1, 0);
                        } else {
                    ...
                    case 3: // Create Private
                    {
                        ...
                        AudioManager_PlayCh("assets/audio/sfx/config_select.ogg", 0.33, 1, 0);
                        int result = multiplayer_host(mp, self->nameInput, 1);
                        if(result != MPAPI_OK)
                        {
                            AudioManager_PlayCh("assets/audio/sfx/denied.ogg", 0.5, 1, 0);
                        } else {
...
int SceneMulti_CreateLobby_Work(void* _self, SDL_Window* window, SDL_Renderer* renderer, double deltaTime)
{
    ...
    FontManager_Render(renderer, "Monaco", height/16, width/2, height/2.95, "Lobby Name", Centered, Centered, (SDL_Color){255, 255, 255, 255});
    ...
    int finalWidth = 0;
    int finalHeight = 0;
    SDL_Rect textRect = FontManager_Render(renderer, "Monaco", height/16, width/2, start, "Create as Public", Centered, Centered, (SDL_Color){255, 255, 164, 255});
    if(self->selPos == 2)
    {
    ...
    start += height/20;
    textRect = FontManager_Render(renderer, "Monaco", height/16, width/2, start, "Create as Private", Centered, Centered, (SDL_Color){255, 192, 128, 255});
    if(self->selPos == 3)
    {
        finalWidth = textRect.w;
        finalHeight = start;
