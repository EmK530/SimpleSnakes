...
{
    SceneMulti_EnterPrivate* scene = malloc(sizeof(SceneMulti_EnterPrivate));
    scene->sceneInfo = malloc(sizeof(GenericScene));
    scene->sceneInfo->context = scene;
    scene->sceneInfo->prepFunction = SceneMulti_EnterPrivate_Prepare;
    scene->sceneInfo->workFunction = SceneMulti_EnterPrivate_Work;
    scene->sceneInfo->inputFunction = SceneMulti_EnterPrivate_OnInput;
    ...
    returnData->sceneId = SCENE_MULTI_ENTERPRIVATE;
    returnData->sceneName = strdup("Multiplayer_EnterPrivate");
...
{
    SceneMulti_EnterPrivate* self = (SceneMulti_EnterPrivate*)_self;
...
int SceneMulti_EnterPrivate_OnInput(void* _self, SDL_Window* window, SDL_Renderer* renderer, SDL_Event* event)
{
    ...
    if(self->selPos == 1 && event->type == SDL_TEXTINPUT)
    {
        ...
        if(textLen == 1 && self->typePos + textLen <= 6)
        {
            memcpy(self->codeInput+self->typePos, event->text.text, textLen);
...
int SceneMulti_EnterPrivate_Work(void* _self, SDL_Window* window, SDL_Renderer* renderer, double deltaTime)
{
    ...
    FontManager_Render(renderer, "Monaco", height/16, width/2, height/2.95, "Lobby ID", Centered, Centered, (SDL_Color){255, 255, 255, 255});
    SDL_Rect inputRect = {width/2,height/2.65,height/4,height/22};
    ...
    if(self->typePos == 0)
    {
        ...
        FontManager_Render(renderer, "Monaco", height/16, width/2, height/2.625, self->codeInput, Centered, Start, (SDL_Color){255, 255, 255, 255});
    }
    ...
    int finalWidth = 0;
    int finalHeight = 0;
    SDL_Rect textRect = FontManager_Render(renderer, "Monaco", height/16, width/2, start, "Search", Centered, Centered, (SDL_Color){255, 255, 164, 255});
    if(self->selPos == 2)
    {
